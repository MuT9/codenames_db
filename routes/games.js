...
gamesRouter.route('/games/:id/hint')
    .get(async (req, res, next) => {
        try {
            res.status(200);
            res.json(await Game.findById(req.params.id).select('hintWord hintCount -_id'));
        } catch (e) {
            next(e);
        }
    })
    .post(async (req, res, next) => {
        try {
            ...
            res.status(200);
            res.json(await game.setHint(req.body.hintWord, req.body.hintCount));
        } catch (e) {
            next(e);
        }
    });
...
gamesRouter.route('/games/:id/move')
    .post(async (req, res, next) => {
        try {
            const game = await Game.findById(req.params.id);
            const word = await game.makeMove(req.body.word);
